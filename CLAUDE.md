...
│   ├── src/
│   │   ├── App.tsx
│   │   ├── main.tsx           # React root mount, I18nProvider setup
│   │   ├── ws/                # WebSocket client, message handling
│   │   ├── hotkeys/           # Hotkey system — agnostic to source
│   │   ├── components/
│   │   ├── locales/           # i18n catalogs (PO source + compiled TS)
│   │   │   └── en/
│   │   │       ├── messages.po   # Source catalog — editable in POEdit
│   │   │       └── messages.ts   # Compiled catalog — generated, gittracked
│   │   └── types/             # TypeScript types matching Go wire format
│   ├── public/
│   │   └── manifest.json      # PWA manifest
│   ├── index.html
│   ├── lingui.config.ts       # LinguiJS config — locales, catalog paths, format
│   ├── vite.config.mts
│   ├── tsconfig.json
│   └── package.json
│
...
All UI strings are wrapped with [LinguiJS](https://lingui.dev) macros. Uses PO/gettext format (POEdit-compatible), compile-time extraction, ~10.4 kB runtime.

- `t` from `@lingui/core/macro` — plain strings: `` t`Save` ``, `` t`Universe ${id} already exists` ``
- `Trans` from `@lingui/react/macro` — JSX content: `<Trans>BLACKOUT ACTIVE</Trans>`
- `lingui.config.ts` — locale list, catalog paths, PO format
- `vite.config.mts` — `@lingui/babel-plugin-lingui-macro` in `react()` Babel plugins
- `main.tsx` — loads compiled catalog, wraps app in `<I18nProvider>`
- `src/locales/en/messages.po` — source catalog (committed, editable in POEdit)
- `src/locales/en/messages.ts` — compiled catalog (committed, regenerated by `lingui compile --typescript`)

**Workflow for adding/changing strings:**

```bash
# 1. Use t`` or <Trans> in component code
# 2. Extract messages to PO
task i18n:extract     # or: pnpm --filter ui i18n:extract
# 3. Compile PO to importable TS
task i18n:compile     # or: pnpm --filter ui i18n:compile
# 4. Commit both messages.po and messages.ts
```

**Adding a new locale:** edit `locales` array in `ui/lingui.config.ts`, run extract, translate the new `.po` file, compile, and add the catalog load in `main.tsx`.
